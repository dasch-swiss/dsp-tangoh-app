describe('Resource Editing', () => {
    ...
    it('edit an anything:Thing', () => {
        ...
        cy.get('#1 .value_container').eq(7).should('have.text', 'test')

        cy.get('#1 .value_container').eq(7)
            .find('img[src="http://0.0.0.0:3335//app/icons/16x16/edit.png"]').click()

        cy.get('#1 .value_container').eq(7).find('input.propedit').clear().type('testtext')

        cy.get('#1 .value_container').eq(7)
            .find('img[src="http://0.0.0.0:3335//app/icons/16x16/save.png"]').click()

        cy.get('#1 .value_container').eq(7).should('have.text', 'testtext')
